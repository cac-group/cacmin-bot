...
/**
 * Tests for the gambling/roll game system
 */
...
vi.mock("../../src/services/unifiedWalletService", () => ({ SYSTEM_USER_IDS: { BOT_TREASURY: -1, SYSTEM_RESERVE: -2, UNCLAIMED: -3, GIVEAWAY_ESCROW_BASE: -1000 } }));
vi.mock("../../src/services/userService", () => ({ createUser: vi.fn(), userExists: vi.fn(() => false), ensureUserExists: vi.fn() }));
vi.mock("../../src/utils/precision", () => ({
	AmountPrecision: {
		format: (n: number) => n.toFixed(6), parseUserInput: (s: string) => parseFloat(s), validateAmount: (n: number) => n, isGreaterOrEqual: (a: number, b: number) => a >= b,
		fromDbMicro: (micro: number) => micro / 1_000_000, toDbMicro: (juno: number) => Math.round(juno * 1_000_000),
	},
}));

import { SYSTEM_USER_IDS } from "../../src/services/unifiedWalletService";
import { get, execute } from "../../src/database";
import { checkWin, generateRollNumber, initializeRollSystem, getServerSeedCommitment, rotateServerSeed, MIN_BET, MAX_BET, WIN_MULTIPLIER } from "../../src/commands/gambling";
